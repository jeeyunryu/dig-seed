...
class TFDecoder(BaseDecoder):
    ...
    def _attention(self, trg_seq, tgt_lens, src, src_mask=None, cls_query_attn_maps=None, trg_word_emb=None):
        if trg_word_emb is not None:
            ...
            trg_embedding = self.trg_word_emb(trg_seq) # [64, 25] -> [64, 25, 512]
        # TODO: debug
    ...
    def forward_train(self, feat, out_enc, targets, tgt_lens, img_metas, cls_query_attn_maps, trg_word_emb):
        r"""
        Args:
            feat (None): Unused. 인코더의 출력값
            out_enc (Tensor): Encoder output of shape :math:`(N, D_m, H, W)` 인코더의 출력값!!
        ...
        """
        # targets, tgt_lens GT임!
        # out_enc: [N, len, C]
        src_mask = None
        targets = targets.to(out_enc.device) # [64, 256, 512]
        # Insert the start_idx to the first step
        init_query = torch.full((out_enc.size(0), 1), self.start_idx).type_as(targets) # self.start_idx: number of classes 97 # [64, 25]
        # [64, 1]
        query_targets = torch.cat([init_query, targets], dim=-1)[:, :-1] # GT임

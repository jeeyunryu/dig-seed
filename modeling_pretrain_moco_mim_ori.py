...
class PatchNet(nn.Module):
  ...
  def forward(self, seq_x, return_attn_map=False):
    ...
    x = F.adaptive_avg_pool2d(x, (1, self.num_windows)).permute(0, 2, 3, 1).squeeze(1) # [B, num_windows, C] # patch head 부분!!!
...
class MoCo_ViT(nn.Module):
  def __init__(self,
               ...
               label_smoothing=0.,
               use_pix_projector=True,
               pretrained_cfg=None): # add a small conv net before contrastive learning to give some localization information
    """
  ...
  def contrastive_loss(self, q, k, return_acc=False, temp=1., hard_k=None):
      ...
      logits = torch.einsum('nc,mc->nm', [q, k]) / self.T # row vector들 간 유사도 계산
  ...
  def forward(self, image, aug_image, vis_mask_pos, m, only_mim_on_ori_img=True,):
    ...
    if self.use_moco_target:
      patches = self.patch_extractor(encoder_output) # patch head
    ...
    if self.use_pixel_target:
      ...
      if only_mim_on_ori_img:
        vis_out = dec_out_list[0][vis_mask_pos_list[0]].reshape(B//2, -1, C) # true인 것 만을 남김
...
def accuracy(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        pred = pred.t()
        correct = pred.eq(target.view(1, -1).expand_as(pred)) # pred[0]: 256개의dml query와 제일 유사도가높은 키들

        res = []
        for k in topk:
            correct_k = correct[:k].reshape(-1).float().sum(0, keepdim=True)
            res.append(correct_k.mul_(100.0 / batch_size)) # a = a * b in place multiplication
        return res
...
class label_smooth_loss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        true_dist.scatter_(1, target.data.unsqueeze(1), self.positive) # one-hot vector 넣어줌!! 

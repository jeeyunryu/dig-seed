...
for embed, label, path, index, imgkey in zip(tsne_result, labels_rslt, imgpaths, char_indices, imgkeys):
    grouped_features[label].append(embed)
    grouped_paths[label].append(path)
    grouped_char_idx[label].append(index)
    grouped_imgkeys[label].append(imgkey)
...
for label, feats in grouped_features.items():
    ...
    for idx in outlier_idx:
        ...
        out_imgkeys.append(grouped_imgkeys[label][idx])

out_imgkeys = list(set(out_imgkeys))
with open('out_imgkeys_exif.txt', 'w') as f:

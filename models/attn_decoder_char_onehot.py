...
class AttentionRecognitionHead(nn.Module):
  ...
  """
  def __init__(self, num_classes, in_planes, sDim, attDim, max_len_labels, args):
    ...
    self.decoder = DecoderUnit(sDim=sDim, xDim=in_planes, yDim=num_classes, attDim=attDim, args=args)
  ...
  def forward(self, x, embed=None):
    if self.training:
      return self.forward_train(x, embed)
    else:
      return self.sample(x, embed)

  def forward_train(self, x, embed=None):
    x, targets, lengths = x
    batch_size = x.size(0)
...
class DecoderUnit(nn.Module):
  def __init__(self, sDim, xDim, yDim, attDim, args):
    ...
    self.embed_fc = nn.Linear(97, self.sDim)
